...
export class LLMService {
  ...
  async generateApiFromNaturalLanguage(input: NaturalLanguageInput): Promise<LLMResult<GeneratedApiSpec>> {
    try {
      // コンテンツポリシー違反チェック
  ...
  async validateApiSafety(code: string): Promise<LLMResult<SafetyValidationResult>> {
    try {
      // 危険なパターンをチェック
  ...
  private containsInappropriateContent(description: string): boolean {
    const inappropriateKeywords = [
      'マルウェア', 'malware', 'virus', 'hack', 'exploit',
      'illegal', 'drug', 'weapon', 'violence'
    ]
  ...
  private isValidApiSpec(spec: unknown): spec is GeneratedApiSpec {
    return (
      ...
      ['GET', 'POST', 'PUT', 'DELETE', 'PATCH'].includes((spec as GeneratedApiSpec).method)
    )
  }
}