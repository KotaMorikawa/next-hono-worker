  ...
  LLMService: vi.fn().mockImplementation(() => ({
    generateApiFromNaturalLanguage: vi.fn().mockResolvedValue({
      ...
      data: {
        ...
        method: 'GET',
        price: '0.01',
        generatedCode: 'export default function handler() { return { weather: "sunny" } }',
...
describe('API生成エンドポイント（修正版）', () => {
  ...
  describe('POST /internal/generator/create', () => {
    it('有効な自然言語入力でAPI生成に成功する', async () => {
      ...
      const data = await res.json() as { success: boolean; data: typeof mockApiData }
      expect(data.success).toBe(true)
      expect(data.data.name).toBe('Mock Weather API')
    })
  ...
  describe('GET /internal/generator/list', () => {
    it('認証ユーザーのAPI一覧を取得できる', async () => {
      ...
      const data = await res.json() as { success: boolean; data: typeof mockApis }
      expect(data.success).toBe(true)
  ...
  describe('GET /internal/generator/:id', () => {
    it('有効なIDでAPI詳細を取得できる', async () => {
      ...
      // Assert
      expect(res.status).toBe(200)
      const data = await res.json() as { success: boolean; data: typeof mockApi }
      expect(data.success).toBe(true)
