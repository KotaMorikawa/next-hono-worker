...
export class GeminiClient {
  async generateCode(input: NaturalLanguageInput): Promise<GeneratedApiSpec> {
    ...
    const mockSpec: GeneratedApiSpec = {
      ...
      endpoint,
      method,
      price: input.expectedPrice || priceMapping[input.complexityLevel] || '0.05',
  ...
  private inferHttpMethod(description: string): 'GET' | 'POST' | 'PUT' | 'DELETE' | 'PATCH' {
    const lowerDesc = description.toLowerCase()
    if (lowerDesc.includes('取得') || lowerDesc.includes('get') || lowerDesc.includes('fetch')) {
      return 'GET'
    }
    if (lowerDesc.includes('作成') || lowerDesc.includes('create') || lowerDesc.includes('post')) {
      return 'POST'
    }
    if (lowerDesc.includes('更新') || lowerDesc.includes('update') || lowerDesc.includes('put')) {
      return 'PUT'
    }
  ...
  private generateHonoCode(endpoint: string, method: string, input: NaturalLanguageInput): string {
...
export default app`
  ...
  private generateDocumentation(endpoint: string, method: string, input: NaturalLanguageInput): string {
    const price = input.expectedPrice || '0.01'
